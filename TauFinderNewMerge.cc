...
void TauFinder::processEvent( LCEvent * evt ) 
{
  ...
  if(reco_tau_vec.size() > 1)
      ...
      for (unsigned int i=0; i<reco_tau_vec.size(); i++)
	{
	  ...
	  //dynamic merge criteria
	  float tau_dynamic_pt = std::sqrt(tau_i[0]*tau_i[0] + tau_i[1]*tau_i[1]);
...
{
  if (pt <= 20.0) return 0.3;
  else if (pt > 20.0 && pt < 60.0) return std::max(6.0/pt, 0.1);
  else return 0.1;
}
